                ...
                <div class="flex flex-col text-sm gap-1 lg:gap-0 xl:gap-1">
                  <div class="flex flex-row items-center gap-1">
                    ...
                    <div class="flex flex-row">
                      @for($i = 0; $i < 5; $i++)
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 xl:size-6">
                  ...
                  <button class="w-fit py-1 px-4 mt-1 xl:mt-3 bg-[#014421] hover:bg-green-800 text-white text-sm rounded-md"> View wordcloud </button>
                </div>
              </div>
            </div>
          </div>
          <div class="flex justify-start rounded-md bg-white shadow w-full gap-4" x-data="{ deleteAccountModalOpen: false }">
            <div class="flex flex-col py-6 px-8 lg:px-5 lg:py-4 xl:py-6 xl:px-8">
              ...
              <div class="flex flex-col gap-2">
                ...
                <button @click="deleteAccountModalOpen = true; document.body.style.overflow = 'hidden';" class="w-fit font-medium px-2 sm:px-3 py-1 text-sm bg-white text-[#7b1113] border border-[#7b1113] rounded-md hover:bg-rose-300">Delete Account</button>
              </div>
            </div>
            <!-- DELETE ACCOUNT MODAL -->
            <div @keydown.escape.window="deleteAccountModalOpen = false; document.body.style.overflow = 'auto';" x-data="{ confirm: '', errors: {} }" x-show="deleteAccountModalOpen" x-transition:enter.duration.25ms class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div class="bg-white p-6 rounded-lg w-5/6 md:w-1/2 lg:w-1/3 relative">
                      <div class="flex flex-col items-center gap-2 sm:gap-3">
                        ...
                        <button @click="deleteAccountModalOpen = false; document.body.style.overflow = 'auto';" class="absolute top-4 right-4 p-2 hover:bg-gray-100 hover:rounded-full">
                      ...
                      <div class="mt-5 flex gap-2">
                          <button @click="deleteAccountModalOpen = false; delete errors.confirm; confirm = ''; document.body.style.overflow = 'auto';" class="px-2 sm:px-3 py-1.5 text-sm border rounded-md hover:bg-slate-200 ml-auto">Cancel</button>
                          <button 
                              @click="
                                  if (confirm === 'CONFIRM') {
                                      $wire.deleteAccount();
        ...
        <div class="flex flex-col gap-4">
          <div class="bg-white rounded-lg shadow w-full">
            <div class="flex flex-col py-6 px-8 justify-center" x-data="{ contact: $wire.entangle('contact'), originalContact:  '{{ $user->contact_number }}', constituent: '{{ $user->constituent }}', selectedConstituent: '{{ $user->constituent }}' , college: '{{ $user->college }}', selectedCollege: '{{ $user->college }}', degprog: '{{ $user->degree_program }}', selectedDegprog: '{{ $user->degree_program }}', degProgs: {{ json_encode($degprogs) }}, isModalOpen: false, infoModalOpen: false, errors: {} }">
              <div class="flex flex-row items-center">
                ...
                <button @click="isModalOpen = true; document.body.style.overflow = 'hidden';" class="ml-auto py-1 px-3 bg-[#014421] hover:bg-green-800 text-white text-sm rounded-md"> Change role </button>
              </div>
              <p class="text-sm">You are logged in as <span class="font-semibold"> {{ $user->role === 'customer' ? 'Customer' : 'Provider' }} </span></p>
              <!-- MODAL -->
              <div @keydown.escape.window="isModalOpen = false; document.body.style.overflow = 'auto';" x-show="isModalOpen" x-transition:enter.duration.25ms class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div class="bg-white p-6 rounded-lg w-4/6 md:w-6/12 xl:w-4/12 relative">
                      ...
                      @if($user->role === 'customer')
                        <p class="text-sm mt-2 md:mt-5 sm:ml-2 text-justify">By changing role to <span class="font-semibold">Provider</span>, you will be able to:</p>
                      ...
                      @else
                        <p class="text-xs md:text-sm mt-2 md:mt-5 sm:ml-2 text-justify">
                          By changing your role to <span class="font-semibold">Customer</span>, you will be able to:
                        </p>
                        <ul class="text-sm mt-2 md:mt-4 list-disc list-inside ml-5">
                      ...
                      <div class="mt-5 flex justify-end gap-2">
                          ...
                          <button @click="isModalOpen = false; document.body.style.overflow = 'auto';" wire:click="changeRole" class="px-2 sm:px-3 py-1 sm:py-1.5 text-sm bg-[#014421] text-white rounded-md hover:bg-green-800">Confirm</button>
                      </div>
                  </div>
              </div>
              ...
              <div @keydown.escape.window="infoModalOpen = false; document.body.style.overflow = 'auto';" x-show="infoModalOpen" x-transition:enter.duration.25ms class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div class="bg-white p-6 rounded-lg w-5/6 md:w-2/3 lg:w-1/2 2xl:w-1/3 relative">
                      ...
                      <div class="mt-2 md:mt-5 border border-gray-200 rounded-md p-2">
                        ...
                        <button @click="infoModalOpen = false; document.body.style.overflow = 'auto';" class="absolute top-4 right-4 p-2 hover:bg-gray-100 hover:rounded-full">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="#000000" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                          </svg>
                        </button>
                      ...
                      <div class="mt-5 flex justify-end gap-2">
                          <button @click="infoModalOpen = false; document.body.style.overflow = 'auto';" class="px-2 sm:px-3 py-1.5 text-sm bg-white text-black  rounded-md hover:bg-slate-200 border hover:border-slate-200 hover:text-black">Cancel</button>
                          <button @click="infoModalOpen = false; document.body.style.overflow = 'auto';" wire:click="saveInfoChanges" class="px-2 sm:px-3 py-1 sm:py-1.5 text-sm bg-[#014421] text-white rounded-md hover:bg-green-800">Confirm</button>
                      </div>
                  </div>
              </div>
            </div>
          ...
          <div x-data="{ current_password: $wire.entangle('current_password'), new_password: $wire.entangle('new_password'), confirm_new_pass: '', showCurrentPassword: false, showNewPassword: false, showConfirmPassword: false, wrong_password: $wire.entangle('wrong_password'), passModalOpen: $wire.entangle('open_modal') }" class="mb-4">
            <div class="bg-white rounded-lg shadow py-6 px-8">
              ...
              <div class="mt-6 flex justify-start">
                <button class="font-medium py-2 px-3 bg-[#014421] enabled:hover:bg-green-800 disabled:bg-gray-300 disabled:cursor-not-allowed text-white text-sm rounded-md" :disabled="(new_password !== confirm_new_pass && new_password.length > 0 && confirm_new_pass.length > 0) || (new_password.length < 8 && new_password.length > 0) || new_password.length > 40 || current_password === '' || (!current_password || !new_password || !confirm_new_pass) || !/[A-Z]/.test(new_password)" type="button" @click="$wire.checkPassword(); document.body.style.overflow = 'hidden';" >Save changes</button>
              </div>
              <div @keydown.escape.window="passModalOpen = false; document.body.style.overflow = 'auto';" x-show="passModalOpen" x-transition:enter.duration.25ms class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div class="bg-white p-6 rounded-lg w-5/6 md:w-2/3 lg:w-7/12 xl:w-1/3 relative">
                      <div class="flex flex-col">
                        <div class="flex flex-row items-center gap-2 sm:gap-3">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#014421" class="size-6 md:size-7">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        ...
                        <button @click="passModalOpen = false; document.body.style.overflow = 'auto';" class="absolute top-4 right-4 p-2 hover:bg-gray-100 hover:rounded-full">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="#000000" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                          </svg>
                        </button>
                      ...
                      <div class="mt-5 flex justify-end gap-2">
                          <button @click="passModalOpen = false; document.body.style.overflow = 'auto';" class="px-2 sm:px-3 py-1.5 text-sm border rounded-md hover:bg-slate-200 ml-auto">Cancel</button>
                          <button @click="passModalOpen = false; document.body.style.overflow = 'auto';" wire:click="savePassChanges" class="px-2 sm:px-3 py-1 sm:py-1.5 text-sm bg-[#014421] text-white rounded-md hover:bg-green-800">Confirm</button>
                      </div>
                  </div>
              </div>
            </div>
