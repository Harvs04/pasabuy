...
<div class="bg-black bg-opacity-50 fixed inset-0 flex items-center justify-center z-50 font-poppins" x-data="{ order: '', orders: $wire.entangle('orders'), order_list_modal_open: false, notes: $wire.entangle('notes'),  order_info_modal_open:false }">
    <div @keydown.escape.window="orderItemModalOpen = false" class="bg-white p-4 md:p-6 rounded-lg w-5/6 md:w-5/12">
        <div class="flex flex-row items-center gap-3">
            ...
            <button @click="orderItemModalOpen = false" class="ml-auto hover:bg-gray-100 hover:rounded-full p-2">
        ...
        <div class="mt-4 border border-gray-200 p-4 rounded-md">
            ...
            <div class="flex flex-row gap-1 items-center mt-4 mb-1 relative text-gray-800 ">
                <div class="w-full flex flex-row gap-1 relative items-center">
                    ...
                    <div class="flex flex-col w-full absolute bottom-6 left-0 z-50 border rounded-lg bg-gray-100 text-gray-800 shadow p-3 max-h-40 overflow-y-scroll" @click.outside="order_list_modal_open = false" x-show="order_list_modal_open">
                        ...
                        <div class="mt-2 md:mt-4">
                            <p x-show="orders.length === 0" class="mt-2 text-center text-xs md:text-sm">--- No orders yet ---</p>
                            <ul x-show="orders.length > 0" class="list-decimal ml-4 break-all">
                                <template x-for="(order_item, index) in orders" :key="index">
                                    <li class="ml-4 hover:bg-gray-300 hover:rounded-md text-xs md:text-sm ">
                ...
                <button class="ml-auto p-1 text-gray-400 text-xs md:text-sm rounded-full hover:bg-gray-200" 
                    @click="if (order) { orders.push(order); order = ''; }">
            ...
            <textarea
                ...
                rows="1"
                @keydown.enter.prevent="if (order) { orders.push(order); order = ''; }">
            </textarea>
