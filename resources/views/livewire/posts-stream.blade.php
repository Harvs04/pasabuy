            ...
            @foreach($posts as $post)
                <div class="bg-white border sm:rounded-md text-gray-800 text-sm relative" x-data="{ openComment: false, openList: false, show: true, reportModalOpen: false, post_id: $wire.entangle('post_id'), reported_id: $wire.entangle('reported_id'), exists: $wire.entangle('exists') }">
                        ...
                        <div class="flex flex-row items-start gap-3">
                            ...
                            @php
                                $colorClass = match($post->status) {
                                    ...
                                    'ongoing' => 'bg-yellow-100 text-yellow-800 font-medium px-2.5 py-1 rounded dark:bg-yellow-900 dark:text-yellow-300',
                                    'converted' => 'bg-gray-100 text-gray-800 font-medium px-2.5 py-1 rounded dark:bg-gray-700 dark:text-gray-300', 
                                    'completed' => 'bg-gray-100 text-gray-800 font-medium px-2.5 py-1 rounded dark:bg-gray-700 dark:text-gray-300',
                                    'full' => 'bg-red-100 text-red-800 font-medium px-2.5 py-1 rounded dark:bg-red-900 dark:text-red-300',
                                    'cancelled' => 'bg-red-100 text-red-800 font-medium px-2.5 py-1 rounded dark:bg-red-900 dark:text-red-300'
                                };
                            @endphp
                    ...
                    <div x-show="!show" class="p-2">
                        <button class="flex items-center gap-1.5 justify-center md:justify-end w-full md:w-auto" @click="show = true">
                    ...
                    <div @keydown.escape.window="reportModalOpen = false; document.body.style.overflow = 'auto';"
                        ...
                        <div class="bg-white py-4 px-2 md:px-6 md:py-6 rounded-lg w-5/6 md:w-1/2 lg:w-1/3 relative" @click.outside="reportModalOpen = false; document.body.style.overflow = 'auto';">
                            <div class="flex flex-col items-center gap-2 sm:gap-3" x-data="{ reportPageOne: true, reportPageTwo: false, reportType: '', reportText: $wire.entangle('complaint'), post_id: '{{ $post->id }}', reported_id: '{{ $post->user_id }}' }">
                                ...
                                <div x-show="reportPageTwo" class="w-full flex flex-col gap-0.5">
                                    ...
                                    <div class="mt-5 flex gap-2">
                                        ...
                                        <button x-data="{ disabled: false }" x-bind:disabled="disabled || reportText.trim().length === 0"
