                ...
                <div class="w-5/6 flex flex-row gap-1 ml-auto">
                    <button class="w-4/12 md:w-5/12 py-1.5 hover:bg-gray-200 hover:rounded-md" :class="'{{ $user->role }}' === 'provider' ? 'hidden' : 'block'">
                        <div class="flex flex-row items-center justify-center gap-3">
                            ...
                            <svg wire:loading.class="hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4 md:size-5" :class="openBurger ? 'lg:size-4 xl:size-5': 'size-5'">
                    ...
                    <button @click=" saved = !saved; $wire.savePost({{ $post->id }}, saved).then(() => $wire.call('refreshSavedPosts')).catch(() => saved = originalState).finally(() => loading = false); " :class="saved ? 'text-[#014421]' : ''" class="w-4/12 md:w-5/12 py-1.5 hover:bg-gray-200 hover:rounded-md ">
                        <div class="flex flex-row items-center justify-center gap-3">
                            ...
                            <svg wire:loading.class="hidden" xmlns="http://www.w3.org/2000/svg" :fill="saved ? '#014421' : 'none'" viewBox="0 0 24 24" stroke-width="2" :stroke="saved ? '#014421' : 'currentColor'" class="size-4 md:size-5" :class="openBurger ? 'lg:size-4 xl:size-5': 'size-5'">
