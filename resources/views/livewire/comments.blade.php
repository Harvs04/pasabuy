                ...
                <div class="w-4/6 sm:w-5/6 flex flex-row gap-1 ml-auto">
                    <button @click="liked = !liked; $wire.likePost('{{ $post->id }}', liked).then(() => $wire.call('refresh')); " class="w-4/12 md:w-5/12 py-1.5 hover:bg-gray-200 hover:rounded-md" :class="'{{ $user->role }}' === 'provider' ? 'hidden' : 'block'">
                        <div class="flex flex-row items-center justify-center gap-3" :class="liked ? 'text-[#014421]' : ''">
                            ...
                            <svg wire:loading.class="hidden" wire:target="likePost" xmlns="http://www.w3.org/2000/svg" :fill="liked ? '#014421' : 'none'" viewBox="0 0 24 24" stroke-width="2" :stroke="liked ? '#014421' : 'currentColor'" class="size-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                            </svg>
                            <p class="hidden lg:font-medium lg:block" >I want this too!</p>
                        </div>
                    </button>
                    <button class="w-4/12 md:w-5/12 py-1.5 enabled:hover:bg-gray-200 enabled:hover:rounded-md disabled:bg-gray-200 disabled:text-gray-300 disabled:cursor-not-allowed disabled:rounded-md" :class="{'hidden': role === 'customer' || user_id === post_user_id, 'block': role !== 'customer' && user_id !== post_user_id }" :disabled="pasabuy_points < 80 || post_status === 'converted'" @click="makeTransactionModalOpen = true; document.body.style.overflow = 'hidden';">
                        <div class="flex flex-row items-center justify-center gap-3">
                            ...
                            <p  class="hidden lg:font-medium lg:block" >Make transaction</p>
                        </div>
                    </button>
                    ...
                    <button @click=" saved = !saved; $wire.savePost('{{ $post->id }}', saved).then(() => $wire.call('refresh')); " :class="saved ? 'text-[#014421]' : ''" class="w-4/12 md:w-5/12 py-1.5 hover:bg-gray-200 hover:rounded-md ">
                        <div class="flex flex-row items-center justify-center gap-3">
                            ...
                            <svg wire:loading.class="hidden" wire:target="savePost" xmlns="http://www.w3.org/2000/svg" :fill="saved ? '#014421' : 'none'" viewBox="0 0 24 24" stroke-width="2" :stroke="saved ? '#014421' : 'currentColor'" class="size-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
                            </svg>
                            <p  class="hidden lg:font-medium lg:block" >Save post</p>
                        </div>
                    </button>
                </div>
            </div>
        ...
        <div class="relative">
            ...
            <div class="flex flex-row gap-1 text-gray-600 text-sm items-center">
                ...
                <div class="w-4/6 sm:w-5/6 flex flex-row gap-1 ml-auto">
                    ...
                    x-bind:disabled="pasabuy_points < 80 || !contact_number || !college || !degree_program || post_status !== 'open' || order_count === max_orders">
                        <div class="flex flex-row items-center justify-center gap-3">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                            </svg>
                            <p class="hidden lg:font-medium lg:block" >Order item</p>
                        </div>
                    </button>
        ...
        <div class="py-1">
            <div class="" x-data="{ comment: '', comments: $wire.entangle('comments')  }">
                ...
                <div class="flex flex-row items-center gap-2">
                    <img class="w-8 md:w-9 h-8 md:h-9 rounded-full object-cover border" 
